...
// Removed Course and User imports as they are now accessed via base44 global object
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Badge } from "@/components/ui/badge";
import { Share2, BookOpen, EyeOff, Edit } from "lucide-react"; // Removed Users icon
...
export default function Storefront() {
  ...
  const loadStorefrontData = useCallback(async () => {
    ...
    try {
      ...
      if (creatorEmail) {
        // Updated to use base44.entities.User.filter()
      ...
      // Updated to use base44.entities.Course.filter()
      const creatorCourses = await base44.entities.Course.filter({
  ...
  const handleUnpublishCourse = async (courseId) => {
    if (window.confirm("Are you sure you want to remove this course from your storefront?")) {
      try {
        // Updated to use base44.entities.Course.update()
        await base44.entities.Course.update(courseId, { is_published: false });
            ...
            <div className="flex flex-col items-center text-center md:flex-row md:items-end md:text-left -mt-12 md:-mt-16 space-y-4 md:space-y-0 md:space-x-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-center md:justify-start gap-1.5 mt-2 text-xs md:text-sm text-slate-500">
                  ...
                  {/* Removed students count as courses are free */}
                </div>
              </div>
